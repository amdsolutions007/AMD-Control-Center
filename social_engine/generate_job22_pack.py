#!/usr/bin/env python3
"""Job 22: CyberSecurity Shield (Digital Defense)

Creates the Job 22 asset pack STRICTLY derived from the branded Source Truth image:
  - Source Truth (must exist): social_engine/assets/Job22_CyberSecurity_Shield/Job22_Twin_Master.png
  - Flyer:  social_engine/assets/Job22_CyberSecurity_Shield/Job22_Flyer_Master.png
  - Audio:  social_engine/assets/Job22_CyberSecurity_Shield/Job22_Audio_Master.mp3
  - Video:  social_engine/assets/Job22_CyberSecurity_Shield/Job22_Video_Master.mp4 (still + narration fallback)
  - Prompt: social_engine/assets/Job22_CyberSecurity_Shield/Job22_Veo3_Video_Master_Prompt.txt
...
Usage:
  python3 social_engine/generate_job22_pack.py
  python3 social_engine/generate_job22_pack.py --force
"""
...
def _extract_bible_fields(bible_path: Path) -> tuple[str, str]:
    """Return (audio_script, veo3_prompt) from the Job 22 bible."""
...
def build_flyer(master_png: Path, out_png: Path, force: bool) -> None:
    ...
    # Cyber cue: blue headline accent; keep footer gold.
    ...
    headline_font = _get_font(size=int(h * 0.060))
    subhead_font = _get_font(size=int(h * 0.052))
    no_virus_font = _get_font(size=int(h * 0.040))
    footer_font = _get_font(size=int(h * 0.035))

    def centered_text(y: int, text: str, font, fill, stroke_fill=None) -> tuple[int, int, int, int]:
    ...
    centered_text(int(h * 0.07), headline, headline_font, neon_blue, stroke_fill=shadow)
    centered_text(int(h * 0.14), subhead, subhead_font, neon_blue, stroke_fill=shadow)

    # Red "NO VIRUS." crossed out (thin line only; no box)
    left, top, right, bottom = centered_text(int(h * 0.205), no_virus, no_virus_font, red, stroke_fill=shadow)
    mid_y = (top + bottom) // 2
    draw.line([(left, mid_y), (right, mid_y)], fill=red, width=max(2, int(h * 0.003)))
...
def generate_audio(client: OpenAI, script: str, out_mp3: Path, force: bool) -> None:
    ...
    print("🎙️  Generating Job 22 audio master (Onyx / tts-1-hd)…")
...
def build_video(flyer_png: Path, audio_mp3: Path, out_mp4: Path, force: bool) -> None:
    ...
    print("🎬 Building Job 22 video (still flyer + narration fallback)…")
...
def write_veo3_prompt(job_num: int, job_dir: Path, veo_prompt: str, audio_script: str, force: bool) -> None:
    ...
    if out_file.exists() and not force:
        ...
        return
    if not veo_prompt:
        raise RuntimeError("No Veo3 prompt found in Job 22 bible")
...
def main() -> None:
    ...
    job_dir = repo_root / "social_engine" / "assets" / "Job22_CyberSecurity_Shield"
    bible_path = repo_root / "social_engine" / "jobs_data" / "Job22_CyberSecurity_Shield.md"

    master_png = job_dir / "Job22_Twin_Master.png"
    flyer_png = job_dir / "Job22_Flyer_Master.png"
    audio_mp3 = job_dir / "Job22_Audio_Master.mp3"
    video_mp4 = job_dir / "Job22_Video_Master.mp4"

    print("=" * 70)
    print("JOB 22 COMPLETE PACK (SOURCE TRUTH → FLYER → AUDIO → VIDEO)")
    print("=" * 70)

    if not bible_path.exists():
        raise FileNotFoundError(f"Missing Job 22 bible: {bible_path}")
    ...
    write_veo3_prompt(job_num=22, job_dir=job_dir, veo_prompt=veo_prompt, audio_script=audio_script, force=args.force)

    print("=" * 70)
    print("✅ Job 22 pack complete")
    print("=" * 70)
    print(f"- Source truth: {master_png}")
    print(f"- Flyer:        {flyer_png}")
    print(f"- Audio:        {audio_mp3}")
    print(f"- Video:        {video_mp4}")
    print(f"- Veo3 prompt:  {job_dir / 'Job22_Veo3_Video_Master_Prompt.txt'}")
