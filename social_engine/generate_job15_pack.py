#!/usr/bin/env python3
"""Job 15: Naija Resume Scanner (ATS Optimizer)

Creates the Job 15 asset pack STRICTLY derived from the branded Source Truth image:
  - Source Truth (must exist): assets/Job15_Resume_Scanner/Job15_Twin_Master.png
  - Flyer:  assets/Job15_Resume_Scanner/Job15_Flyer_Master.png
  - Audio:  assets/Job15_Resume_Scanner/Job15_Audio_Master.mp3
  - Video:  assets/Job15_Resume_Scanner/Job15_Video_Master.mp4 (still + narration fallback)
  - Prompt: assets/Job15_Resume_Scanner/Job15_Veo3_Video_Master_Prompt.txt

Branding rules (per Job 15 Bible):
  - Flyer overlay text: GET HIRED. / BEAT THE HR BOT.
  - Footer: AMD SOLUTIONS 007
  - Audio voice: OpenAI TTS, voice="onyx"

Usage:
  python3 generate_job15_pack.py
  python3 generate_job15_pack.py --force
"""
...
def _extract_bible_fields(bible_path: Path) -> tuple[str, str]:
    """Return (audio_script, veo3_prompt) from the Job 15 bible."""
...
def build_flyer(master_png: Path, out_png: Path, force: bool) -> None:
    ...
    headline = "GET HIRED."
    subhead = "BEAT THE HR BOT."
    footer = "AMD SOLUTIONS 007"

    draw = ImageDraw.Draw(img)

    headline_font = _get_font(size=int(h * 0.060), bold=True)
    subhead_font = _get_font(size=int(h * 0.045), bold=True)
...
def generate_audio(client: OpenAI, script: str, out_mp3: Path, force: bool) -> None:
    ...
    print("🎙️  Generating Job 15 audio master (Onyx / tts-1-hd)…")
...
def build_video(flyer_png: Path, audio_mp3: Path, out_mp4: Path, force: bool) -> None:
    ...
    print("🎬 Building Job 15 video (still flyer + narration fallback)…")
...
def write_veo3_prompt(job_num: int, job_dir: Path, veo_prompt: str, audio_script: str, force: bool) -> None:
    ...
    if out_file.exists() and not force:
        ...
        return
    if not veo_prompt:
        raise RuntimeError("No Veo3 prompt found in Job 15 bible")
...
def main() -> None:
    ...
    job_num = 15
    job_dir = base_dir / "assets" / "Job15_Resume_Scanner"
    master = job_dir / "Job15_Twin_Master.png"
    flyer = job_dir / "Job15_Flyer_Master.png"
    audio = job_dir / "Job15_Audio_Master.mp3"
    video = job_dir / "Job15_Video_Master.mp4"
    bible = base_dir / "jobs_data" / "Job15_Resume_Scanner.md"

    script, veo_prompt = _extract_bible_fields(bible)

    print("=" * 70)
    print("JOB 15 COMPLETE PACK (SOURCE TRUTH → FLYER → AUDIO → VIDEO)")
    print("=" * 70)
