#!/usr/bin/env python3
"""Job 20: Telemed Health AI (Smart Health)

Creates the Job 20 asset pack STRICTLY derived from the branded Source Truth image:
  - Source Truth (must exist): social_engine/assets/Job20_Telemed_Health_AI/Job20_Twin_Master.png
  - Flyer:  social_engine/assets/Job20_Telemed_Health_AI/Job20_Flyer_Master.png
  - Audio:  social_engine/assets/Job20_Telemed_Health_AI/Job20_Audio_Master.mp3
  - Video:  social_engine/assets/Job20_Telemed_Health_AI/Job20_Video_Master.mp4 (still + narration fallback)
  - Prompt: social_engine/assets/Job20_Telemed_Health_AI/Job20_Veo3_Video_Master_Prompt.txt
...
Usage:
  python3 social_engine/generate_job20_pack.py
  python3 social_engine/generate_job20_pack.py --force
"""
...
def _extract_bible_fields(bible_path: Path) -> tuple[str, str]:
    """Return (audio_script, veo3_prompt) from the Job 20 bible."""
...
def build_flyer(master_png: Path, out_png: Path, force: bool) -> None:
    ...
    subhead = "AT HOSPITALS."
    tagline = "THE DOCTOR IS ALWAYS IN."
    cta = "DM 'HEALTH'"
    footer = "AMD SOLUTIONS 007"
    ...
    tagline_font = _get_font(size=int(h * 0.035))
    cta_font = _get_font(size=int(h * 0.032))
    ...
    centered_text(int(h * 0.205), tagline, tagline_font, white, stroke_fill=shadow)
    centered_text(h - int(h * 0.115), cta, cta_font, white, stroke_fill=shadow)
    centered_text(h - int(h * 0.080), footer, footer_font, gold, stroke_fill=shadow)
...
def generate_audio(client: OpenAI, script: str, out_mp3: Path, force: bool) -> None:
    ...
    print("🎙️  Generating Job 20 audio master (Onyx / tts-1-hd)…")
...
def build_video(flyer_png: Path, audio_mp3: Path, out_mp4: Path, force: bool) -> None:
    ...
    print("🎬 Building Job 20 video (still flyer + narration fallback)…")
...
def write_veo3_prompt(job_num: int, job_dir: Path, veo_prompt: str, audio_script: str, force: bool) -> None:
    ...
    if out_file.exists() and not force:
        ...
        return
    if not veo_prompt:
        raise RuntimeError("No Veo3 prompt found in Job 20 bible")
...
def main() -> None:
    ...
    job_dir = repo_root / "social_engine" / "assets" / "Job20_Telemed_Health_AI"
    bible_path = repo_root / "social_engine" / "jobs_data" / "Job20_Telemed_Health_AI.md"

    master_png = job_dir / "Job20_Twin_Master.png"
    flyer_png = job_dir / "Job20_Flyer_Master.png"
    audio_mp3 = job_dir / "Job20_Audio_Master.mp3"
    video_mp4 = job_dir / "Job20_Video_Master.mp4"

    print("=" * 70)
    print("JOB 20 COMPLETE PACK (SOURCE TRUTH → FLYER → AUDIO → VIDEO)")
    print("=" * 70)

    if not bible_path.exists():
        raise FileNotFoundError(f"Missing Job 20 bible: {bible_path}")
    ...
    write_veo3_prompt(job_num=20, job_dir=job_dir, veo_prompt=veo_prompt, audio_script=audio_script, force=args.force)

    print("=" * 70)
    print("✅ Job 20 pack complete")
    print("=" * 70)
    print(f"- Source truth: {master_png}")
    print(f"- Flyer:        {flyer_png}")
    print(f"- Audio:        {audio_mp3}")
    print(f"- Video:        {video_mp4}")
    print(f"- Veo3 prompt:  {job_dir / 'Job20_Veo3_Video_Master_Prompt.txt'}")
