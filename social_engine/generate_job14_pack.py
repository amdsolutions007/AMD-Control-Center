#!/usr/bin/env python3
"""Job 14: TIL Knowledge Bank (AI Education)

Creates the Job 14 asset pack STRICTLY derived from the branded Source Truth image:
  - Source Truth (must exist): assets/Job14_TIL_KnowledgeBank/Job14_Twin_Master.png
  - Flyer:  assets/Job14_TIL_KnowledgeBank/Job14_Flyer_Master.png
  - Audio:  assets/Job14_TIL_KnowledgeBank/Job14_Audio_Master.mp3
  - Video:  assets/Job14_TIL_KnowledgeBank/Job14_Video_Master.mp4 (still + narration fallback)
  - Prompt: assets/Job14_TIL_KnowledgeBank/Job14_Veo3_Video_Master_Prompt.txt

Branding rules (per Job 14 Bible):
  - Flyer overlay text: DOWNLOAD KNOWLEDGE. / LEARN IN MINUTES.
  - Footer: AMD SOLUTIONS 007
  - Audio voice: OpenAI TTS, voice="onyx"

Usage:
  python3 generate_job14_pack.py
  python3 generate_job14_pack.py --force
"""
...
def _extract_bible_fields(bible_path: Path) -> tuple[str, str]:
    """Return (audio_script, veo3_prompt) from the Job 14 bible."""
...
def build_flyer(master_png: Path, out_png: Path, force: bool) -> None:
    ...
    headline = "DOWNLOAD KNOWLEDGE."
    subhead = "LEARN IN MINUTES."
    footer = "AMD SOLUTIONS 007"
...
def generate_audio(client: OpenAI, script: str, out_mp3: Path, force: bool) -> None:
    ...
    print("🎙️  Generating Job 14 audio master (Onyx / tts-1-hd)…")
...
def build_video(flyer_png: Path, audio_mp3: Path, out_mp4: Path, force: bool) -> None:
    ...
    print("🎬 Building Job 14 video (still flyer + narration fallback)…")
...
def write_veo3_prompt(job_num: int, job_dir: Path, veo_prompt: str, audio_script: str, force: bool) -> None:
    ...
    if out_file.exists() and not force:
        ...
        return
    if not veo_prompt:
        raise RuntimeError("No Veo3 prompt found in Job 14 bible")
...
def main() -> None:
    ...
    job_num = 14
    job_dir = base_dir / "assets" / "Job14_TIL_KnowledgeBank"
    master = job_dir / "Job14_Twin_Master.png"
    flyer = job_dir / "Job14_Flyer_Master.png"
    audio = job_dir / "Job14_Audio_Master.mp3"
    video = job_dir / "Job14_Video_Master.mp4"
    bible = base_dir / "jobs_data" / "Job14_TIL_KnowledgeBank.md"

    script, veo_prompt = _extract_bible_fields(bible)

    print("=" * 70)
    print("JOB 14 COMPLETE PACK (SOURCE TRUTH → FLYER → AUDIO → VIDEO)")
    print("=" * 70)
