#!/usr/bin/env python3
"""Job 18: Buqiazo Thrift (AI Fashion E-Commerce)

Creates the Job 18 asset pack STRICTLY derived from the branded Source Truth image:
  - Source Truth (must exist): social_engine/assets/Job18_Reborn_Thrift_Store/Job18_Twin_Master.png
  - Flyer:  social_engine/assets/Job18_Reborn_Thrift_Store/Job18_Flyer_Master.png
  - Audio:  social_engine/assets/Job18_Reborn_Thrift_Store/Job18_Audio_Master.mp3
  - Video:  social_engine/assets/Job18_Reborn_Thrift_Store/Job18_Video_Master.mp4 (still + narration fallback)
  - Prompt: social_engine/assets/Job18_Reborn_Thrift_Store/Job18_Veo3_Video_Master_Prompt.txt

Branding rules (global):
  - Palette: Black + 24K Gold (#D4AF37)
    - Minimalist full-bleed, gold/white typography (no borders/frames)
  - Footer: AMD MEDIA SOLUTIONS
  - Audio voice: OpenAI TTS, voice="onyx"

Usage:
  python3 social_engine/generate_job18_pack.py
  python3 social_engine/generate_job18_pack.py --force
"""
...
def _extract_bible_fields(bible_path: Path) -> tuple[str, str]:
    """Return (audio_script, veo3_prompt) from the Job 18 bible."""
...
def generate_audio(client: OpenAI, script: str, out_mp3: Path, force: bool) -> None:
    ...
    print("🎙️  Generating Job 18 audio master (Onyx / tts-1-hd)…")
...
def build_video(flyer_png: Path, audio_mp3: Path, out_mp4: Path, force: bool) -> None:
    ...
    print("🎬 Building Job 18 video (still flyer + narration fallback)…")
...
def write_veo3_prompt(job_num: int, job_dir: Path, veo_prompt: str, audio_script: str, force: bool) -> None:
    ...
    if out_file.exists() and not force:
        ...
        return
    if not veo_prompt:
        raise RuntimeError("No Veo3 prompt found in Job 18 bible")
...
def main() -> None:
    ...
    job_dir = repo_root / "social_engine" / "assets" / "Job18_Buqaizo_Thrift_Store"
    legacy_job_dir = repo_root / "social_engine" / "assets" / "Job18_Reborn_Thrift_Store"
    bible_path = repo_root / "social_engine" / "jobs_data" / "Job18_Reborn_Thrift_Store.md"

    # Backward-compatible fallback: if the canonical folder is missing the source truth,
    # use the legacy folder (older naming) to avoid breaking automation.
    canonical_master = job_dir / "Job18_Twin_Master.png"
    legacy_master = legacy_job_dir / "Job18_Twin_Master.png"
    if not canonical_master.exists() and legacy_master.exists():
        job_dir = legacy_job_dir

    master_png = job_dir / "Job18_Twin_Master.png"
    flyer_png = job_dir / "Job18_Flyer_Master.png"
    audio_mp3 = job_dir / "Job18_Audio_Master.mp3"
    video_mp4 = job_dir / "Job18_Video_Master.mp4"

    print("=" * 70)
    print("JOB 18 COMPLETE PACK (SOURCE TRUTH → FLYER → AUDIO → VIDEO)")
    print("=" * 70)

    if not bible_path.exists():
        raise FileNotFoundError(f"Missing Job 18 bible: {bible_path}")
    ...
    write_veo3_prompt(job_num=18, job_dir=job_dir, veo_prompt=veo_prompt, audio_script=audio_script, force=args.force)

    print("=" * 70)
    print("✅ Job 18 pack complete")
    print("=" * 70)
    print(f"- Source truth: {master_png}")
    print(f"- Flyer:        {flyer_png}")
    print(f"- Audio:        {audio_mp3}")
    print(f"- Video:        {video_mp4}")
    print(f"- Veo3 prompt:  {job_dir / 'Job18_Veo3_Video_Master_Prompt.txt'}")
