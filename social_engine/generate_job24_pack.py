#!/usr/bin/env python3
"""Job 24: AMD Synergy Master Hub (The CEO Dashboard)

Creates the Job 24 asset pack STRICTLY derived from the branded Source Truth image:
  - Source Truth (must exist): social_engine/assets/Job24_Synergy_Master_Hub/Job24_Twin_Master.png
  - Flyer:  social_engine/assets/Job24_Synergy_Master_Hub/Job24_Flyer_Master.png
  - Audio:  social_engine/assets/Job24_Synergy_Master_Hub/Job24_Audio_Master.mp3
  - Video:  social_engine/assets/Job24_Synergy_Master_Hub/Job24_Video_Master.mp4 (still + narration fallback)
  - Prompt: social_engine/assets/Job24_Synergy_Master_Hub/Job24_Veo3_Video_Master_Prompt.txt
...
Usage:
  python3 social_engine/generate_job24_pack.py
  python3 social_engine/generate_job24_pack.py --force
"""
...
def _extract_bible_fields(bible_path: Path) -> tuple[str, str]:
    """Return (audio_script, veo3_prompt) from the Job 24 bible."""
...
def build_flyer(master_png: Path, out_png: Path, force: bool) -> None:
    ...
    headline = "THE WORLD IS AUTOMATED."
    subhead = "ONE DASHBOARD. TOTAL CONTROL."
    footer = "AMD SOLUTIONS 007"
    ...
    subhead_font = _get_font(size=int(h * 0.035))
    ...
    centered_text(int(h * 0.10), headline, headline_font, gold, stroke_fill=shadow)
    centered_text(int(h * 0.18), subhead, subhead_font, white, stroke_fill=shadow)
    centered_text(h - int(h * 0.08), footer, footer_font, gold, stroke_fill=shadow)
...
def generate_audio(client: OpenAI, script: str, out_mp3: Path, force: bool) -> None:
    ...
    print("🎙️  Generating Job 24 audio master (Onyx / tts-1-hd)…")
...
def build_video(flyer_png: Path, audio_mp3: Path, out_mp4: Path, force: bool) -> None:
    ...
    print("🎬 Building Job 24 video (still flyer + narration fallback)…")
...
def write_veo3_prompt(job_num: int, job_dir: Path, veo_prompt: str, audio_script: str, force: bool) -> None:
    ...
    if out_file.exists() and not force:
        ...
        return
    if not veo_prompt:
        raise RuntimeError("No Veo3 prompt found in Job 24 bible")
...
def main() -> None:
    ...
    job_dir = repo_root / "social_engine" / "assets" / "Job24_Synergy_Master_Hub"
    bible_path = repo_root / "social_engine" / "jobs_data" / "Job24_Synergy_Master_Hub.md"

    master_png = job_dir / "Job24_Twin_Master.png"
    flyer_png = job_dir / "Job24_Flyer_Master.png"
    audio_mp3 = job_dir / "Job24_Audio_Master.mp3"
    video_mp4 = job_dir / "Job24_Video_Master.mp4"

    print("=" * 70)
    print("JOB 24 COMPLETE PACK (SOURCE TRUTH → FLYER → AUDIO → VIDEO)")
    print("=" * 70)

    if not bible_path.exists():
        raise FileNotFoundError(f"Missing Job 24 bible: {bible_path}")
    ...
    write_veo3_prompt(24, job_dir, veo_prompt, audio_script, force=args.force)

    print("=" * 70)
    print("✅ Job 24 pack complete")
    print("=" * 70)
    print(f"- Source truth: {master_png}")
    print(f"- Flyer:        {flyer_png}")
    print(f"- Audio:        {audio_mp3}")
    print(f"- Video:        {video_mp4}")
    print(f"- Veo3 prompt:  {job_dir / 'Job24_Veo3_Video_Master_Prompt.txt'}")
