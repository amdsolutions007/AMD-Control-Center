#!/usr/bin/env python3
"""Job 17: SkyCap AI (Market Intelligence)

Creates the Job 17 asset pack STRICTLY derived from the branded Source Truth image:
  - Source Truth (must exist): social_engine/assets/Job17_SkyCap_Market_Data/Job17_Twin_Master.png
  - Flyer:  social_engine/assets/Job17_SkyCap_Market_Data/Job17_Flyer_Master.png
  - Audio:  social_engine/assets/Job17_SkyCap_Market_Data/Job17_Audio_Master.mp3
  - Video:  social_engine/assets/Job17_SkyCap_Market_Data/Job17_Video_Master.mp4 (still + narration fallback)
  - Prompt: social_engine/assets/Job17_SkyCap_Market_Data/Job17_Veo3_Video_Master_Prompt.txt
...
Usage:
  python3 social_engine/generate_job17_pack.py
  python3 social_engine/generate_job17_pack.py --force
"""
...
def _extract_bible_fields(bible_path: Path) -> tuple[str, str]:
    """Return (audio_script, veo3_prompt) from the Job 17 bible."""
...
def generate_audio(client: OpenAI, script: str, out_mp3: Path, force: bool) -> None:
    ...
    print("🎙️  Generating Job 17 audio master (Onyx / tts-1-hd)…")
...
def build_video(flyer_png: Path, audio_mp3: Path, out_mp4: Path, force: bool) -> None:
    ...
    print("🎬 Building Job 17 video (still flyer + narration fallback)…")
...
def write_veo3_prompt(job_num: int, job_dir: Path, veo_prompt: str, audio_script: str, force: bool) -> None:
    ...
    if out_file.exists() and not force:
        ...
        return
    if not veo_prompt:
        raise RuntimeError("No Veo3 prompt found in Job 17 bible")
...
def main() -> None:
    ...
    job_dir = repo_root / "social_engine" / "assets" / "Job17_SkyCap_Market_Data"
    bible_path = repo_root / "social_engine" / "jobs_data" / "Job17_SkyCap_Market_Data.md"

    master_png = job_dir / "Job17_Twin_Master.png"
    flyer_png = job_dir / "Job17_Flyer_Master.png"
    audio_mp3 = job_dir / "Job17_Audio_Master.mp3"
    video_mp4 = job_dir / "Job17_Video_Master.mp4"

    print("=" * 70)
    print("JOB 17 COMPLETE PACK (SOURCE TRUTH → FLYER → AUDIO → VIDEO)")
    print("=" * 70)

    if not bible_path.exists():
        raise FileNotFoundError(f"Missing Job 17 bible: {bible_path}")
    ...
    write_veo3_prompt(job_num=17, job_dir=job_dir, veo_prompt=veo_prompt, audio_script=audio_script, force=args.force)

    print("=" * 70)
    print("✅ Job 17 pack complete")
    print("=" * 70)
    print(f"- Source truth: {master_png}")
    print(f"- Flyer:        {flyer_png}")
    print(f"- Audio:        {audio_mp3}")
    print(f"- Video:        {video_mp4}")
    print(f"- Veo3 prompt:  {job_dir / 'Job17_Veo3_Video_Master_Prompt.txt'}")
